...
def catalog(s, article):
    ...
    name_pattern = "<name>(.*?)</name>"                     # extract the info from the tags
    price_pattern = "<prx>(.*?)</prx>"
    qty_pattern = "<qty>(.*?)</qty>"
    sp = s.split("</prod>")                                 # each element is separated by <prod></prod>
    output = []
    
    for el in sp:
        if article in el:
            name, price, qty = re.findall(name_pattern, el)[0], re.findall(price_pattern, el)[0], re.findall(qty_pattern, el)[0]
            print(name, price, qty)
            output.append(f"{name} > prx: ${price} qty: {qty}")         # match the three if search term matches, append to output
    
    return "\r\n".join(output) if output else "Nothing"                 # join with carriage return + newline