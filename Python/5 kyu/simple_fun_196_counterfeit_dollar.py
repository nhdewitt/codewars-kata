...
https://www.codewars.com/kata/58c61a1e8a452631c5000003/train/python
"""

def counterfeit_dollar(results):
    real = set()                                                            # keep track of real coins and counterfeit coins
    counterfeit = set()
    last_left = last_right = ""                                             # keep track of the weighing that was not even
    last_outcome = ""
    
    for result in results:
        left, right, outcome = result.split()                               # for each of the results, if even, they are all real, add to the real set
        if outcome == "even":
            real |= set(left + right)
        else:
            last_left, last_right, last_outcome = left, right, outcome      # otherwise, they are potentially counterfeit, add to that set
            counterfeit |= set(left + right)
            
    counterfeit -= real                                                     # set operations, subtract all known real coins from the potential counterfeit coins
    if len(counterfeit) > 1:                                                # if there's more than one potential counterfeit, we can't know for certain
        return "???"
    
    coin = counterfeit.pop()
    side = last_left if last_outcome == "up" else last_right                # if it's up, the left side is heavier and the coin is on that side and vice versa
    weight = "heavy" if coin in side else "light"                           # if the coin is in the heavy side it is heavy, otherwise it's light
    
    return f"{coin} is the counterfeit coin and it is {weight}."