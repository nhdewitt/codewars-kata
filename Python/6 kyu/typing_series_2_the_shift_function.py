...
https://www.codewars.com/kata/64b63c024979c9001f9307e4/train/python
"""

import re

shift = re.compile(r'\[shift\](.)')
holdshift = re.compile(r'\[holdshift\](.*?)\[unshift\]')

def type_out(strng):
    strng = shift.sub(lambda m: m.group(1).upper(), strng)
    
    return holdshift.sub(lambda m: m.group(1).upper(), strng).replace("[unshift]","")