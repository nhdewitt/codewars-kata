...
def up_array(arr):
    if not arr or min(arr) < 0 or max(arr) > 9:
        return None
    conversion = int("".join(map(str, arr)))
    conversion += 1
    output = []
    if len(str(conversion)) < len(arr):
        temp_str = str(conversion).zfill(len(arr))
        return [int(d) for d in temp_str]
    else:
        return [int(d) for d in str(conversion)]