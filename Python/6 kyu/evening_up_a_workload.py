...
https://www.codewars.com/kata/56431c04ed1454a35d00003b/train/python
"""
...
def split_workload(workload):
    ...
    left = 0
    for i in range(1, n):                   # starting with the entire workload on one side, remove an element at a time and compare the diffs
        left += workload[i - 1]
        right = total - left
        diff = abs(left - right)
        if diff < best_diff:                # if the diff is smaller, track the newest low idx and diff
