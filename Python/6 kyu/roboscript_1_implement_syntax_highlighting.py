...
def span(c, s):
    if c == "F":
        return f'<span style="color: pink">{s}</span>'
    if c == "L":
...
def highlight(code):
    output = []
    sp = re.findall(r'R+|F+|L+|\d+|\(|\)', code)                # regex: split groups of R, F, L, digits, separate out ( and )
    for s in sp:
        print(s)
        if "F" in s:
            output.append(span("F", s))
        elif "L" in s:
            output.append(span("L", s))
        elif "R" in s:
            output.append(span("R", s))
        elif s.isdigit():
            output.append(span("1", s))
        else:
            output.append(s)
    return "".join(output)