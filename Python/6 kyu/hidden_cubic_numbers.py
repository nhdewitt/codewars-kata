...
def is_sum_of_cubes(s):
    matches = re.findall(r'\d{1,3}', s)
    lucky = [int(match) for match in matches if is_cubic(match)]
    
    if not lucky: return "Unlucky"

    return f"{' '.join(str(l) for l in lucky)} {sum(lucky)} Lucky"