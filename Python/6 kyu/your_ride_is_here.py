...
def ride(group, comet):
    val = {k: i for i, k in enumerate(ascii_lowercase, start=1)}        # Create a mapping of letters to their respective values
    g = [val[ch] for ch in group.lower()]
    c = [val[ch] for ch in comet.lower()]
    
    return 'GO' if (prod(g) % 47) == (prod(c) % 47) else 'STAY'