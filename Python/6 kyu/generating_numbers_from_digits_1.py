...
def proc_arr(arr):
    count = Counter(arr)
    total = factorial(len(arr))         # unique permutations = len(arr)! / factorial(frequency) of each digit
    for frequency in count.values():
        total //= factorial(frequency)
    
    low = ''.join(sorted(arr))          # lowest is the string sorted
    high = low[::-1]                    # highest is the reverse
    
    return [total, int(low), int(high)] # conversion to int() will remove any leading zeroes