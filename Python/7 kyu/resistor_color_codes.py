...
def decode_resistor_colors(bands):
    ...
    resistance = (RESISTORS[band[0]] * 10 + RESISTORS[band[1]]) * 10 ** RESISTORS[band[2]]
    if resistance < 1000:
        return f"{resistance} ohms, {tolerance}%"
    if resistance < 1000000:
        if resistance % 1000 == 0:
            return f"{resistance // 1000}k ohms, {tolerance}%"
        else:
            return f"{resistance / 1000:.1f}k ohms, {tolerance}%"
    else:
        if resistance % 1000000 == 0:
            return f"{resistance // 1000000}M ohms, {tolerance}%"
        else:
            return f"{resistance / 1000000:.1f}M ohms, {tolerance}%"