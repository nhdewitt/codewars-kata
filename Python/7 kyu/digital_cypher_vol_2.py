...
alphabet = {index: letter for index, letter in enumerate(ascii_lowercase, start=1)}

def decode(code, key):
    message_code = []
    idx = 0
    for _, v in enumerate(code):
        message_code.append(alphabet[v - int(str(key)[idx % len(str(key))])])
        idx += 1
