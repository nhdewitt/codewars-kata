...
def who_is_online(friends):
    ...
    for friend in friends:
        if friend['status'] == "online" and friend['last_activity'] <= 10:
            statuses['online'].append(friend['username'])
        elif friend['status'] == "offline":
            statuses['offline'].append(friend['username'])
        elif friend['status'] == "online" and friend['last_activity'] > 10:
            statuses['away'].append(friend['username'])
    
    return {k: v for k, v in statuses.items() if v}